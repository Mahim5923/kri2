...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if (oauth && userData) {
      try {
        ...
        // Store user in sessionStorage for app access
        sessionStorage.setItem('user', JSON.stringify(user));
        alert(`Welcome${user.name ? `, ${user.name}` : ''}! Signed in via ${oauth}.`);
  ...
  const handleOAuth = (provider) => {
    // OAuth redirect URLs (would need backend OAuth configuration to work)
    const oauthUrls = {
      Google: 'http://127.0.0.1:5000/api/auth/google',
      LinkedIn: 'http://127.0.0.1:5000/api/auth/linkedin'
    };
    
    // In production, redirect to OAuth provider
    // For now, shows info and proceeds to setup
